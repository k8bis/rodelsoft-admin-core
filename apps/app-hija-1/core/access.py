...
from core.admin_security import is_system_admin
...
def has_app_client_admin_scope(
    db: Session,
    username: str,
    app_id: int,
    client_id: int,
...
def resolve_role(
    ...
    """
    Rol contextual oficial:
    - system_admin
    - app_client_admin
    - member

    IMPORTANTE:
    enforce_access(...) debe correr antes de esta función.
    """
    if is_system_admin(db, username):
        return ("system_admin", True, False, False)

    if has_app_client_admin_scope(db, username, app_id, client_id):
        return ("app_client_admin", False, True, False)

    return ("member", False, False, True)
...
def build_session_context_payload(
    ...
    role, is_system_admin_flag, is_app_client_admin, is_member = resolve_role(
        db, username, app_id, client_id
    )
    ...
    return {
        ...
        "role": role,
        "is_system_admin": is_system_admin_flag,
