...
from schemas.auth import Login, PublicSessionContextResponse
...
@router.get("/public/session-context", response_model=PublicSessionContextResponse)
def public_session_context(
    request: Request,
    user: str = Depends(verify_token),
    db: Session = Depends(get_db),
    x_app_id: int | None = Header(alias="X-App-Id", default=None),
    ...
    return build_session_context_payload(db, user, app_id, client_id)
