...
# Configuración
# =========================================================
PORTAL_ROOT_URL = os.getenv("PUBLIC_BASE_URL", "/")
...
def build_dynamic_proxy_url(request: Request, public_url: str, entry_path: str, app_id: int, client_id: int) -> str:
    """
    Construye URL dinámica usando:
    - PUBLIC_BASE_URL como base pública canónica del shell
    - public_url como ruta pública configurable de la app en BD
    - entry_path como ruta interna inicial

    Ejemplo:
      PUBLIC_BASE_URL = http://45.33.3.22:8088
      public_url = /ext/stocks/
      entry_path = /
      => http://45.33.3.22:8088/ext/stocks/?app_id=6&client_id=1
    """
    raw_public = (public_url or "").strip()
    raw_entry = (entry_path or "/").strip()

    if not raw_public.startswith("/"):
        raise HTTPException(
            status_code=500,
            detail="public_url inválido para dynamic_proxy: debe iniciar con '/'",
        )
    ...
    base_url = f"{public_base}{raw_public.rstrip('/')}"
...
def launch(request: Request, app_id: int, client_id: int):
    ...
    try:
        ...
        if launch_mode == "dynamic_proxy":
            final_url = build_dynamic_proxy_url(request, public_url, entry_path, app_id, client_id)
        else:
